...
function setInvoiceData(element) {
    const partyName = element.getAttribute('data-name');
    const partyMobile = element.getAttribute('data-id');
    ...
    // document.getElementById('firstChar').appendChild(div);
    document.getElementById('partyName').textContent = "PartyName :  " + partyName;
    document.getElementById('partyMobile').textContent = "InvoiceNumber :  " + partyMobile;
    document.getElementById('partyId').value = partyId;

    document.getElementById('drawer-right-select-invoice');
    document.getElementById('noPartySelectedLayout').style.display = 'none';
    document.getElementById('showSelectedPartyLayout').style.display = 'block';

    //we call api to get the bill details bu selected bill
    fetch('/users/transactions/get-bill/' + partyId)
        .then(response => response.json())
        .then(data => {
            newBill = data;
            //print the data of the bill
            console.log(newBill);
            console.log(newBill.productTransactions);
            // set all producttransaction data in the select item drawer where product id is matched
            const productTransactionsList = document.getElementsByClassName('QuantitySelectedLayout');
            console.log(productTransactionsList);
            Array.from(productTransactionsList).forEach(item => {
                    const productId = item.querySelector('#itemAddEditBtnFromDrawer').getAttribute('data-product-id');
                    console.log("productId", productId);
                    // find in bill product transaction where product id is matched
                    let productTransaction;
                    newBill.productTransactions.forEach(product => {
                        console.log(product.product.id);
                        if (product.product.id.toString() === productId.toString()) {
                            console.log(product.product.id);
                    ...
                    if (productTransaction) {
                        console.log("success", productTransaction);
                        console.log("success", item);
                        item.querySelector('#quantityInput').value = productTransaction.unit;
                        console.log(productTransaction.unit);
                        item.querySelector('#itemPrice').textContent = '₹ ' + productTransaction.amount;
                        item.querySelector('#itemDiscount').textContent = '₹ ' + productTransaction.discount;
                        item.querySelector('#itemFinalPrice').textContent = '₹ ' + (productTransaction.amount - productTransaction.discount);
                    }
                }
            );
            setData();
